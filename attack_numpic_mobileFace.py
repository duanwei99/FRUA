...
class PyFAT:
    ...
    def generate(self, time,num_pic):
        ...
        for i in enumerate(a):
            ...
            wolf_feats = []
            for i,image in enumerate(org_img[rand+1:rand+num_pic]):
        ...
        np.save('log/num_pic/num_'+str(num_pic)+'diff.npy',diff)
        # summary = "mobileFace Model w1:w2 = 1:100 detla=10/127.5 iter2000 clip  cosine_decay_with_warmup"
        with open("log/num_pic/num_pic_log.txt",'a') as fp2:
            # print('summary %s' % (summary), file=fp2)
            print('num_pic %d' %(num_pic), end='\t', file=fp2)
