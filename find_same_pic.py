...
if __name__ == "__main__":
    ...
    with logSaver('e.txt'):
        ...
        face_id = []
        id_map = []
        for f_path, dir_name, f_names in os.walk(r'../data/lfw/'):
            if bool(f_path[4:]):
        ...
        for index, vic_name in enumerate(vic_names):
            all_image_paths = glob.glob('../data/lfw/' + vic_name + '/*.*')
            for aip in all_image_paths:
                shutil.copyfile(aip, 'picture/' + str(index) + '/' + os.path.basename(aip))
        print()