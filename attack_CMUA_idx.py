...
class PyFAT:
    ...
    def generate(self, time):
        ...
        att_imgs = np.load('/data/hdd/duanwei/FRUA/some-resources-master/feature_lfw.npy')
        feat_img = np.load("feature1.npy")
        vic_imgs = np.load('/data/hdd/duanwei/FRUA/some-resources-master/feature_lfw.npy')
        ...
        org_img = []
        org_img_sphere = []
        for i, image in tqdm(enumerate(att_imgs)):
        ...
        vic_feats2 = []
        vic_feats3 = []
        for i, image in tqdm(enumerate(vic_imgs)):
            vic_image = torch.Tensor(image.copy()).unsqueeze(0).permute(0, 3, 1, 2).to(self.device)
            vic_image.div_(255).sub_(0.5).div_(0.5)
            vic_feat = self.model1.forward(vic_image)
            vic_feat = vic_feat.cpu().detach().numpy().flatten()
            vic_feats1.append(vic_feat)

            vic_feat = self.model2.forward(vic_image)
            vic_feat = vic_feat.cpu().detach().numpy().flatten()
            vic_feats2.append(vic_feat)

            im1_resize = torch_resize(vic_img)
            vic_feat = self.model3.forward(im1_resize)
            vic_feat = vic_feat.cpu().detach().numpy().flatten()
            vic_feats3.append(vic_feat)
        ...
        diff = np.load("/data/hdd/duanwei/FRUA/some-resources-master/stage2/output/04-27-16-34cross Model without spherefacediff.npy")
        diff = diff / 127.5
        diff1 = torch.Tensor(diff.copy()).unsqueeze(0).permute(0, 3, 1, 2).to(self.device)
        # torch_resize1 = Resize([112, 112])
        # diff = torch_resize1(diff1)


        #model1
        att_feats1 = []
        vic_img = torch.Tensor(feat_img[rand].copy()).unsqueeze(0).permute(0, 3, 1, 2).to(self.device)
        vic_img.div_(255).sub_(0.5).div_(0.5)
        spc_feat = self.model1.forward(vic_img)
        spc_feat = spc_feat.cpu().detach().numpy().flatten()
        # spc_feat = np.load("spec_feat.npy")
        for _,image in tqdm(enumerate(org_img)):
            att_feat = self.model1.forward(image + diff1)
            att_feat = att_feat.cpu().detach().numpy().flatten()
            att_feats1.append(att_feat)

        num_succ1 = 0
        num_sup1 = 0
        num_fail1 = 0
        for i in range(len(att_feats1)):
            if cos_sim(att_feats1[i],spc_feat) > 0.3:
                num_succ1 = num_succ1 + 1
            elif cos_sim(att_feats1[i],spc_feat) > cos_sim(att_feats1[i],vic_feats1[i]):
                num_sup1 = num_sup1 + 1
            else:
                num_fail1 += 1
        ...
        vic_img.div_(255).sub_(0.5).div_(0.5)
        # spc_feat = self.model1.forward(vic_img)
        spc_feat = self.model2.forward(vic_img)
        spc_feat = spc_feat.cpu().detach().numpy().flatten()
        # spc_feat = np.load("spec_feat.npy")
        for _,image in tqdm(enumerate(org_img)):
            att_feat = self.model2.forward(image + diff1)
            att_feat = att_feat.cpu().detach().numpy().flatten()
            att_feats2.append(att_feat)

        num_succ2 = 0
        num_sup2 = 0
        num_fail2 = 0
        for i in range(len(att_feats2)):
            if cos_sim(att_feats2[i],spc_feat) > 0.3:
                num_succ2 +=  1
            elif cos_sim(att_feats2[i],spc_feat) > cos_sim(att_feats2[i],vic_feats2[i]):
                num_sup2 +=  1
            else:
                num_fail2 += 1
        ...
        vic_img = torch.Tensor(feat_img[rand].copy()).unsqueeze(0).permute(0, 3, 1, 2).to(self.device)
        vic_img.div_(255).sub_(0.5).div_(0.5)
        vic_img = torch_resize(vic_img)
        spc_feat = self.model3.forward(vic_img)
        spc_feat = spc_feat.cpu().detach().numpy().flatten()
        # spc_feat = np.load("spec_feat.npy")
        diff1 = torch_resize(diff1)
        for _, image in tqdm(enumerate(org_img_sphere)):
            att_feat = self.model3.forward(image + diff1)
            att_feat = att_feat.cpu().detach().numpy().flatten()
            att_feats3.append(att_feat)

        num_succ3 = 0
        num_sup3 = 0
        num_fail3 = 0
        for i in range(len(att_feats3)):
            if cos_sim(att_feats3[i], spc_feat) > 0.3:
                num_succ3 += 1
            elif cos_sim(att_feats3[i], spc_feat) > cos_sim(att_feats2[i], vic_feats3[i]):
                num_sup3 += 1
            else:
                num_fail3 += 1
        ...
        summary = "cross Model vaild - Spherenet"
        ...
        with open('stage2/log/'+str(time)+summary +"_log.txt",'w') as fp2:
            ...
            print('num_success,num_super,num_fail: %d %d %d' %(num_succ1,num_sup1,num_fail1), file=fp2)
            print('success_ratio %f' %((num_succ1 + num_sup1) / float(num_succ1 +num_sup1 + num_fail1)), file=fp2)
            print('=====================================',file=fp2)
            print('num_success,num_super,num_fail: %d %d %d' % (num_succ2, num_sup2, num_fail2), file=fp2)
            print('success_ratio %f' % ((num_succ2 + num_sup2) / float(num_succ2+ num_sup2 + num_fail2)), file=fp2)
            print('====================================', file=fp2)
            print('num_success,num_super,num_fail: %d %d %d' % (num_succ3, num_sup3, num_fail3), file=fp2)
            print('success_ratio %f' % ((num_succ3 + num_sup3) / float(num_succ3 + num_sup3 + num_fail3)), file=fp2)
            print('=====================================', file=fp2)
            # print(L2,file=fp2)
            # print('w1 and w2 %d %d   the ratio is   %f:' %(w1,w2,w2/w1), file=fp2)
            print('the diff max and min is %f %f' %(diff1.max(),diff1.min()), file=fp2)
